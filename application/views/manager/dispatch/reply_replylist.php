...
<div class="pageheader">
  <h2><i class="fa fa-bars"></i> 派单系统 <span>回复列表</span>
      <?php
      if(!empty($cnrs)){
  ...
  <div class="breadcrumb-wrapper">
    <ol class="breadcrumb">
      ...
      <li class="active"><a href="<?php echo base_url($admin_path.'/dispatcher/article_list');?>">回复列表</a></li>
      <?php
      if(!empty($cnrs)){
          ...
          <?php if(!empty($rs)):?>
              <?php foreach($rs as $rs_row):?>
                  <tr>
                      ...
                      <td><?php switch($rs_row['Cisdanger']){
                              case 0:
                                  echo '未知';
                                  break;
                              case -1:
                      ...
                      <td>
                          <button class="btn btn-white btn-xs btn-margin" onclick="javascript:window.location.href='<?php echo base_url($admin_path.'/dispatcher/select_member/replydispatch/'.$rs_row['order_id']) ?>'">评论</button>
                      </td>
                      <td>
                          <button class="btn btn-white btn-xs btn-margin" onclick="javascript:window.location.href='<?php echo base_url($admin_path.'/dispatcher/select_member/replypositive/'.$rs_row['order_id'])?>'">正负面</button>
                      </td>
                  </tr>
              <?php
              endforeach;
