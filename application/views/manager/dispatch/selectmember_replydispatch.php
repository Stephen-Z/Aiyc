            ...
            <?php if(!empty($rs)):?>
                <?php foreach($rs as $rs_row):?>
                    <tr>
                        ...
                        <td>评论回复(id)：<?php echo $reply_id ?></td>
                        <td>
                            <button class="btn btn-white btn-xs btn-margin"  type="button" onclick="postSelect(<?php echo $rs_row['user_id'] ?>,<?php echo $reply_id ?>)"  >选择</button>
                        </td>
                    </tr>
                    <?php
                endforeach;
...
<script>
    ...
    function postSelect(membersID,articleID){
        $.ajax({
            type: "POST",
            url: "<?php echo base_url($admin_path.'/dispatcher/select_member/replycommentselected');?>",
            dataType: 'json',
            ...
            success:
                function(data){
                    if(data=='1'){
                        ...
                        window.location.href='/manager/dispatcher/reply/replylist';
                    }else{
