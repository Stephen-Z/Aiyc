...
<div class="pageheader">
    <h2><i class="fa fa-bars"></i> 回复评论列表
        <?php
        if(!empty($cnrs)){
            ...
            <?php if(!empty($rs)):?>
                <?php foreach($rs as $rs_row):?>
                    <tr>
                        ...
                        <td style="width: 25%;"><a target="_blank" href="<?php echo $rs_row['share_url'] ?>"><?php echo $rs_row['title']?></a></td>
...
<script>
    function postInfo(articleID,userID,Status){
        ...
        $.ajax({
            type: "POST",
            async: true,
            url: "<?php echo base_url($admin_path.'/article/comment/updatereplystatus');?>",
            dataType: 'json',
