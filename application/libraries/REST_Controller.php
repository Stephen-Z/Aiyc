...
abstract class REST_Controller extends CI_Controller {
    ...
    const MANAGER_TEMPLATE_PATH = 'manager'; //后台模板路径
    const MANAGER_PATH = 'manager'; //后台路径
    const SYSTEM_STYLE_PATH = 'public/system'; //系统样式路径

    const WEB_TEMPLATE_PATH = 'web'; //前端模板路径
    const WEB_STYLE_PATH = 'public/web'; //前端样式路径
    ...
    private function simulateLogin() {
        ...
        if($this->uri->segment(1, 0)=='user'){
            if($this->uri->segment(2, 0)=='login' or $this->uri->segment(2, 0)=='reg'){
            }else{
                if(empty($_SESSION['user']['account'])){
        ...
        //管理员登陆判断
        if($this->uri->segment(1, 0)=='manager'){
            if($this->uri->segment(2, 0)!='auth'){
                if(empty($_SESSION['admin']['name'])){
                    redirect(base_url('/manager/auth'));
                }
            }
        }
